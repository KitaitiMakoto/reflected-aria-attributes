...
class TokenList {
    constructor(element, attribute, value) {
        this._list = new Set(TokenList._splitTokens(value));
...
var ReflectedARIAAttributes = {
    ...
    attachAttributes(element, attributes) {
        for (let attrName of attributes) {
            ...
            if ((desc.value === "ID reference list") || (desc.value === "token list")) {
                this.attachListAttribute(element, attrName, desc.default);
            } else {
    ...
    attachListAttribute(element, attrName, defaultValue) {
        var propName = attrName.replace(/-(\w)/g, (str, c) => c.toUpperCase());
		...
		var attrValue = element.getAttribute(attrName) || defaultValue;
                ...
                if (! cache) {
                    props[listPropName] = cache = {
                        key: attrValue,
                        list: new TokenList(this, attrName, attrValue)
                    };
                }
    ...
    attributes: {
        ...
        "aria-dropeffect": {
            value: "token list",
            set: ["copy", "move", "link", "execute", "popup", "none"],
            default: "none"
        },
        "aria-flowto": {
            value: "ID reference list"
